...
class UserInfo(models.Model):
    username=models.CharField(verbose_name="用户名",max_length=32,db_index=True)#为该字段创建索引，ORM去查询的时候速度更快
...
class Project(models.Model):
    ...
    region = models.CharField(verbose_name='cos桶区域',max_length=32)
    # 查询：可以省事；
    # 增加、删除、修改：无法完成
...
class Wiki(models.Model):
    ...
    #null=True针对的是数据库，blank=True针对的是表单提交时字段是否能为空，两者通常一起使用 related_name是反向查找时使用的代替表名小写_site
    #比如一个wiki对象是w1,可以通过w1.parent得到父文章对象 w1.parent.project w1.parent.title w1.parent.content w1.parent.parent得到父文章信息
    #但是要查找一篇文章比如w1所有的字文章 w1.children  
...
class FileRepository(models.Model):
    #choices参数的类型不是字典，而是元祖里嵌套元祖,列表里嵌套元祖应该也可以
    project = models.ForeignKey(verbose_name='项目',to='Project')
    file_type_choices = (
        (1, '文件'),
        (2, '文件夹'),
    ...
    name = models.CharField(verbose_name='文件夹名称',max_length=32,help_text='文件夹/文件名')
    # key是文件存储在腾讯COS对象存储中的文件名
    key = models.CharField(verbose_name='文件存储在cos中的key', max_length=128, unique=True, null=True, blank=True)
    file_size = models.IntegerField(verbose_name='大小',null=True,blank=True,help_text='字节')
    file_path = models.CharField(verbose_name='文件路径',max_length=255,null=True,blank=True)
    parent = models.ForeignKey(verbose_name='父级目录',to='self',null=True,blank=True,related_name='children')
